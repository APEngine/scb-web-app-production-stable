...
def generate_detailed_report(request): 
    ...
    try:
        ...
        date_from = data["dateRange"]["dateFrom"]
        date_to = data["dateRange"]["dateTo"]
        show_all_status = data["projectStatus"]["showAll"]
        is_approved = data["projectStatus"]["isApproved"]
        
    except Exception as e:
        return HttpResponse(json.dumps({"error": str(e)}), content_type="application/json", status=400)
    
    # Start with a QuerySet of all Horasdedicadastareas instances
    queryset = Horasdedicadastareas.objects.all()

    # Apply the filters
    horasdedicadastareas = queryset.filter(
        Q(codigotarea__programador=user_id) if user_id != "Todos" else Q(),
        Q(aprobadas=is_approved) if not show_all_status else Q(),
        Q(codigoproyecto__codigocliente=client_code) if client_code != "Todos" else Q(),
        Q(codigoproyecto__codigoproyecto=project_code) if project_code != "Todos" else Q(),
        Q(fechacreacion__range=(date_from, date_to))
    )
    ...
    if report_type == 2:        
        ...
        # Fetch all AuthUsers and create a dictionary mapping username to full name
        auth_users = AuthUser.objects.annotate(
            full_name=Concat("first_name", V(" "), "last_name")
        ).values('username', 'full_name')
        
        auth_users_dict = {user['username']: user['full_name'] for user in auth_users}
        clients = Clientes.objects.all().values("codigo_cliente", "nombre_cliente")
        clients_dict = {client['codigo_cliente']: client['nombre_cliente'] for client in clients}
        
        # Iterate over the horasdedicadastareas_values QuerySet
        for item in horasdedicadastareas:
            try:
                client_code_number = int(item.codigoproyecto.codigocliente) if (item.codigoproyecto.codigocliente is not None) else None
            except ValueError:
                client_code_number = None
                
            if item.codigoproyecto is not None and client_code_number is not None:
                nombre_cliente = clients_dict.get(client_code_number)
            else:
                nombre_cliente = "Sin nombre"
            # Get the full name of the AuthUser
            responsable = auth_users_dict.get(item.codigotarea.programador)

            # Create a dictionary with the desired structure
            item_data = {
                "codigoProyecto": item.codigoproyecto.codigoproyecto,
                "nombreProyecto": item.codigoproyecto.nombre,
                "codigoCliente": item.codigoproyecto.codigocliente if item.codigoproyecto.codigocliente is not None else "Sin Código",
                "nombreCliente": nombre_cliente if nombre_cliente is not None else "Sin Cliente",
                "nombreTarea": item.codigotarea.nombre,
                "descripcion": item.descripcion,
                "responsable": responsable,
                "horasReales": item.horasdedicadas,
                "estado": "Aprobada" if (item.aprobadas==1) else "Pendiente",
                "ultimaActualizacion": item.fechacreacion.strftime("%Y-%m-%d %H:%M:%S"),
                "tipoProyecto": Proyectos.objects.filter(codigoproyecto=item.codigoproyecto.codigoproyecto).values("tipoproyecto").first()["tipoproyecto"]
            }
...
def retrieve_information_from_code(request, project_code, task_code):
    project_name = Proyectos.objects.filter(codigoproyecto=project_code).values("nombre")
    task_name = Tareas.objects.filter(codigotarea=task_code).values("nombre")
    information = {
        "project_name": project_name[0]["nombre"],
        "task_name": task_name[0]["nombre"]
    }
    return JsonResponse(information, content_type="application/json")
    
  
from django.db.models import Subquery, OuterRef  
from datetime import timedelta
@method_decorator(csrf_exempt, name="dispatch")
def retrieve_two_weeks_dedication(request, user_code):
    if request.method == "GET":
        fifteen_days_ago = timezone.now() - timedelta(days=15)
        
        tasks = Tareas.objects.filter(programador=user_code).exclude(prioridad=0).values("codigotarea")
        values = Horasdedicadastareas.objects.filter(
            codigotarea__in=Subquery(tasks),
            fechacreacion__gte=fifteen_days_ago
        ).values(
            "fechacreacion"
        ).annotate(
            total_horasdedicadas=Sum("horasdedicadas")
        )
        values_list = [
            {
                "creation_date": value["fechacreacion"],
                "total_dedicated_hours": value["total_horasdedicadas"]
            }
            for value in values
        ]
        values_list = sorted(values_list, key=lambda item: item["creation_date"])
...
class UserTasks(APIView):
    ...
    def get(self, request, project_code, task_code):
        ...
        data = {"message": "Success", "projects": tasks} if tasks else {"message": "Proyecto no Encontrado"}
        return JsonResponse(data)
    
    def post(self, request, project_code, task_code):
        json_data = json.loads(request.body)
        descripcion = json_data.get("descripcion")
        horas_dedicadas = json_data.get("horas_dedicadas")
        ...
        try:
            project_code_instanced = Proyectos.objects.get(codigoproyecto=project_code)
            task_code_instanced = Tareas.objects.get(codigotarea=task_code)
            Horasdedicadastareas.objects.create(
                codigoproyecto=project_code_instanced, 
                codigotarea=task_code_instanced, 
    ...
    def put(self, request, project_code, task_code, id):
        ...
        task.horasdedicadas = json_data["horas_dedicadas"]
        task.save()
        return Response({"message": "La entrada seleccionada ha sido modificada con éxito"}, status=status.HTTP_200_OK)
