...
from .models import *
from .models import AuthUser, Usuarios, Horasdedicadastareas, Clientes
from .helpers import (
    ...
    try:
        ...
        for project in total_projects_main_information:
            if project is not None:
                ...
                print(project_information[0]["programador"])
                print(project_information[0]["codigocliente"])
                projects_information.append({
                    ...
                    "projectClientOwner": Clientes.objects.filter(codigo_cliente=project_information[0]["codigocliente"])
                                                         .values_list("nombre_cliente", flat=True).first() if project_information[0]["codigocliente"] is not None else None,
                    "expandedInformation": {
                        "administrator": AuthUser.objects.filter(username=project_information[0]["administrador"])
                                                         .annotate(full_name=Concat("first_name", V(" "), "last_name"))
                                                         .values("full_name")[0]["full_name"],
                        "projectLeader": AuthUser.objects.filter(username=project_information[0]["programador"])
                                                         .annotate(full_name=Concat("first_name", V(" "), "last_name"))
                                                         .values("full_name")[0]["full_name"] if project_information[0]["programador"] is not None else None,
