...
from .models import *
from .models import AuthUser, Usuarios, Horasdedicadastareas, Clientes, Tareas
from .helpers import (
...
def create_new_project_task(request):
    ...
    if frontend_form.is_valid():
        ...
        descripcion = data["taskDescription"]
        ...
        if not Tareas.objects.filter(prioridad__gt=0):
            prioridad = 1
        else:
            prioridad = Tareas.objects.filter(prioridad__gt=0).aggregate(Max('prioridad'))['prioridad__max'] + 1
...
def modify_current_tasks_priority(request):
    try:
        ...
        current_task.programador = task_updated_content["userCode"]
...
def approve_specified_tasks(request):
    try:
        ...
        if parcial_status == 0:
            message = f"Las tareas seleccionadas han sido aprobadas con éxito"
        else:
...
def get_all_current_tasks_information(request) -> HttpResponse:
    ...
    ).values("codigoproyecto", "nombre", "tipoproyecto")
        ...
        task_dict = {
            ...
            "tipo_proyecto": next(
                (
                    ...
                    for project_type in codigoproyecto_name
                    if task["codigoproyecto"] == project_type["codigoproyecto"]
                ),
                None,
            )
        }
...
@require_http_methods(["GET"])
def get_filtered_current_tasks_information(request, user_id, project_id) -> HttpResponse:
    
    if user_id == "Todos" and project_id == "Todos":
        tasks = Tareas.objects.filter(aprobada=False)

        details_all_tasks = Tareas.objects.filter(aprobada=False).values(
            "horasproyectadas",
        )
    elif user_id == "Todos" and project_id != "Todos":
        tasks = Tareas.objects.filter(aprobada=False, codigoproyecto=project_id)

        details_all_tasks = Tareas.objects.filter(aprobada=False, codigoproyecto=project_id).values(
            "horasproyectadas",
        )
    elif user_id != "Todos" and project_id == "Todos":
        tasks = Tareas.objects.filter(aprobada=False, programador=user_id)

        details_all_tasks = Tareas.objects.filter(aprobada=False, programador=user_id).values(
            "horasproyectadas",
        )
...
# Project Management Functions - Start -
@csrf_exempt
def get_project_information_for_table_display(request):
    ...
    try:
        ...
        if load_status in [True, 1, "true", "True"]:
        ...
        for dictionary in total_projects_main_information:
            if dictionary not in current_project_codes:
                current_project_codes.append(dictionary)
        
        
        for project in current_project_codes:
            if project is not None:
                ...
                projectClientOwner = None
                if project_information[0]["codigocliente"] is not None and project_information[0]["codigocliente"].strip() != '':
                    projectClientOwner = Clientes.objects.filter(codigo_cliente=project_information[0]["codigocliente"]).values_list("nombre_cliente", flat=True).first()
                ...
                projects_information.append({
                    ...
                    "quotedHours": project_information[0]["horasproyectadas"] or 0,
                    "realHours": real_hours['horasdedicadas__sum'] or 0,
                    "projectClientOwner": projectClientOwner
                })
        ...
        return JsonResponse({"message": str(e), "type": str(type(e))}, status=500)
    except Exception as e:
        return JsonResponse({"message": "An unexpected error occurred", "type": str(type(e))}, status=500)
...
# Clients Management Functions - Start -
# Get information for display on table
@csrf_exempt
def get_client_information_for_table_display(request):
    try:
        clients_information = []
        total_clients = list(Clientes.objects.all().values("codigo_cliente"))
        for client in total_clients:
            if client is not None:
                current_client_information = list(Clientes.objects.filter(codigo_cliente=client["codigo_cliente"]).values())
                
                clients_information.append({
                    "clientCode": int(current_client_information[0]["codigo_cliente"]),
                    "clientName": current_client_information[0]["nombre_cliente"],
                    "clientWebPage": current_client_information[0]["sitio_web"],
                    "clientEmail":current_client_information[0]["email_contacto"],
                    "expandedInformation": {
                        "clientDirection":current_client_information[0]["direccion"],
                        "clientPhoneContact":current_client_information[0]["telefono_contacto"],
                    }
                })
        return JsonResponse(clients_information, safe=False)
            
    except Exception as e:
        return JsonResponse({"message":e}, status=201)

# Get available codes for new clients
@csrf_exempt
def get_all_available_clients_code(request):
    try:
        total_clients = Clientes.objects.all().values("codigo_cliente")
        unavailable_codes = [client['codigo_cliente'] for client in total_clients]
        return JsonResponse(unavailable_codes, safe=False)
    except Exception as e:
        print(f"error: {e}")

# Get clients short information
@require_http_methods(["GET"])
def get_short_client_list(request):
    try:
        clients = Clientes.objects.only("codigo_cliente", "nombre_cliente")
        if not clients:
            return JsonResponse({"error": "No se encontraron clientes"}, status=404)
        clients_data = [f"{client.codigo_cliente} - {client.nombre_cliente}" for client in clients]
...
def create_new_client(request):
    ...
    if frontend_form.is_valid():        
        client_code = frontend_form.cleaned_data["clientCode"]
        client_name = frontend_form.cleaned_data["clientName"]
        client_direction = frontend_form.cleaned_data["clientDirection"]
        client_phone_contact = frontend_form.cleaned_data["clientPhoneContact"]
        client_email = frontend_form.cleaned_data["clientEmail"]
        client_web_page = frontend_form.cleaned_data["clientWebPage"]
        
        # Creating a new Clientes instance
        client = Clientes(
            id=client_code,
            codigo_cliente=client_code,
            nombre_cliente=client_name,
            direccion=client_direction,
            email_contacto=client_email,
            telefono_contacto=client_phone_contact,
            sitio_web=client_web_page,
        )
        ...
        return JsonResponse({"message": "Nuevo cliente creado con éxito"}, status=201)

    else:
        return JsonResponse({"error": frontend_form.errors}, status=400) 
    
# Get information for specific client
@csrf_exempt
def get_client_information_for_modification(request):
    try:
        data = json.loads(request.body)
        client = list(Clientes.objects.filter(codigo_cliente=data["clientCode"]).values())
        client_information = []
        client_information.append({
            "clientCode": int(client[0]["codigo_cliente"]),
            "clientName": client[0]["nombre_cliente"],
            "clientWebPage": client[0]["sitio_web"],
            "clientEmail":client[0]["email_contacto"],
            "clientDirection":client[0]["direccion"],
            "clientPhoneContact":client[0]["telefono_contacto"]
        })
        return JsonResponse(client_information, safe=False)
            
    except Exception as e:
        return JsonResponse({"message":e}, status=201)

# Modify Client Selected
@csrf_exempt
def modify_selected_client(request):
    try:
        data = json.loads(request.body)
        client = Clientes.objects.get(codigo_cliente=data["clientCode"])
        client.nombre_cliente = data["clientName"]
        client.direccion = data["clientDirection"]
        client.email_contacto = data["clientEmail"]
        client.telefono_contacto = data["clientPhoneContact"]
        client.sitio_web = data["clientWebPage"]
        client.save()
        return JsonResponse(
            {"message": "Información de cliente modificada con éxito"}, status=200
        )   
    except Exception as e:
        print(e)
        return JsonResponse(
            {"message": f"Problema al modificar la información del cliente: {e}"}, status=400
        )
    
# Clients Management Functions - Finish -